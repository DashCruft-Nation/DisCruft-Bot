...
module.exports.run = async (client, message, args) => {
	if (!message.member.voice.channelID) return message.reply('You need to be in a stage channel!', { allowedMentions: { repliedUser: false } });
	const channel = new Discord.VoiceChannel(message.guild, (await client.api.channels(message.member.voice.channelID).get()));
	if(channel.type !== 'unknown') return message.reply('You need to be in a stage channel!');
	if (client.voice.connections.get(message.guild.id)) client.voice.connections.delete(message.guild.id);
	channel.joinable = true;
	if (!channel) return message.reply('You need to be a stage channel!', { allowedMentions: { repliedUser: false } });
	if (!message.guild.me.hasPermission('CONNECT')) return message.reply(`I don't have \`CONNECT\` permission to connect to \`${channel.name}\``, { allowedMentions: { repliedUser: false } });
	if (!message.guild.me.permissionsIn(channel).has('CONNECT')) return message.reply(`I don't have \`CONNECT\` permission to connect to \`${channel.name}\``, { allowedMentions: { repliedUser: false } });
	if (!channel.joinable || !channel.speakable) return message.reply(`I cannot join/speak in ${channel.toString()}`, { allowedMentions: { repliedUser: false } });
	let joiningmsg = null;
	...
	if (ytdl.validateURL(args[0])) {
		...
		if (!message.guild.me.voice.channelID) {
			joiningmsg = await message.reply(`Joining ${channel.toString()}!`, { allowedMentions: { repliedUser: false } });
...
module.exports.config = {
	name: 'stage',
	desc: 'Songs for stages! `This command is specifically for stages`',
	aliases: ['stages'],
};
async function play(song, voicechannel) {
	...
	if (!voicechannel.guild.me.voice.channel || !voicechannel.guild.me.voice.channelID) {
